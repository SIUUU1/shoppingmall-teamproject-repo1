...
<div id="shop" class="box2">
	<c:forEach var="clothList" items="${clothLists}">
		...
		<p class="b">
			[${cloth_categoryName}] 신상: 
			<a href="<%=request.getContextPath()%>/list.do?cloth_category=${cloth_category}">더보기</a>
		</p>
		<c:forEach var="cloth" items="${clothList}">
			<table class="vhcenter">
				<tr height="30">
					<td rowspan="4" width="100"><a
						href="<%=request.getContextPath()%>/clothContent.do?cloth_id=${cloth.getCloth_id()}&cloth_category=${cloth.getCloth_category()}">
							<img src="<%=request.getContextPath()%>/clothImage/${cloth.getCloth_image()}" class="listimage">
					</a></td>
					...
					<a href="<%=request.getContextPath()%>/clothContent.do?cloth_id=${cloth.getCloth_id()}&cloth_category=${cloth.getCloth_category()}"
						class="b"> ${cloth.getCloth_name()}</a></td>
				...
				<tr height="30">
					<td width="350">브랜드 :${cloth.getCloth_brand()}</td>
				</tr>
				<tr height="30">
					<td width="350">
