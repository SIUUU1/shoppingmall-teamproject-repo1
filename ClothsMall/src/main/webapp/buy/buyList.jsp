...
<c:if test="${count > 0}">
	...
	<c:forEach var="i" begin="0" end="${buyLists.size()-1}">
		...
		<div class="buyItem">
		<p class="buyTitle">주문 상품</p>
			<table class="buylist">
				<tr>
					<th width="300" colspan="2">상품</th>
				...
				<tr>
					<td width="150">
					<img src="/ClothsMall/clothImage/${buylist.cloth_image}" class="cartimage" width="80px" height="120px"></td>
					<td width="150">${buylist.cloth_name}</td>
					<td width="200">
					<c:set var="price" value="${buylist.cloth_price}" /> 
					...
					<fmt:formatNumber value="${rPrice}" type="number" pattern="#,##0" />원 <!-- ㄴ view Price 판매가 원화 표시 -->
			...
			<c:forEach var="i" begin="0" end="${receiptLists.size()-1}">
				...
				<c:if test="${receiptList.receipt_id eq thisId}">
					<div class="receiptList">
						<p class="buyTitle">배송지</p>
						<table class="address buylist">
							<tr>
								<th width="150">주문일</th>
								<th width="100">받는 이</th>
								<th width="150">연락처</th>
							...
							<tr>
								<td width="150">${formatter.format(receiptList.buy_date)}</td>
								<td width="100">${receiptList.delivery_name}</td>
								<td width="150">${receiptList.delivery_tel}</td>
								<td width="100">${receiptList.delivery_postal_code}</td>
								<td width="250">${receiptList.delivery_address} ${receiptList.delivery_detailed_address}</td>
								<td width="150">${receiptList.state}</td>
							</tr>
						</table>
					</div>
					<br>
					<p class="buyTitle">결제 상세</p>
